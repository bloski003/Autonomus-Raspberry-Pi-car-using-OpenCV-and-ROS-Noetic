...
# https://github.com/SravanChittupalli/Lane-following-bot-in-Gazebo/blob/master/ros_ws/lane_follow/scripts/utils.py

kernel = np.ones((5 , 5) , np.float32)/25
lower_white = np.array([0,0,200]) #HSV white lower threshold
upper_white = np.array([255,80,255]) #HSV white upper threshold
...
def apply_canny(cv_image):
    ...
    canny = cv2.Canny(blured, 50, 100)  #150, 190
    return canny

def detect_white(cv_image):
    global lower_white , upper_white
    hsv_img = cv2.cvtColor(cv_image , cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(hsv_img , lower_white , upper_white)
    white_edge = apply_canny(mask)
    return white_edge
...
def get_imgs_stacked(scale, imgArray):
	rows = len(imgArray)
	cols = len(imgArray[0])
