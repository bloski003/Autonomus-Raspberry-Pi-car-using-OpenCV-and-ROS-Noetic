...
class Hist_sub:
	...
	def image_callback(self, msg):
		try:
			...
			cv2_img = np.fromstring(msg.data, dtype=np.uint8).reshape(msg.height, msg.width, 3)
			...
			print(curve)
			msg = curve
			pub = rospy.Publisher('/curve_value', Int8, queue_size = 5) #publish curve value [-60,60] as Int8
			rate = rospy.Rate(10)
			pub.publish(msg)
